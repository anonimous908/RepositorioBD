...
| Elemento | Valor |
| :--- | :--- |
| Proyecto | Control Académico e Institucional |
| Versión | 1.0 |
| Fecha | Junio 2026 |
...
| `nombre` | VARCHAR | 50 | NN | Nombre o nombres propios del profesor. |
| `apellido_paterno`| VARCHAR | 50 | NN | Apellido paterno del docente. |
| `apellido_materno`| VARCHAR | 50 | NN | Apellido materno del docente. |
...
| Clave | Regla |
| :--- | :--- |
| **IR-01** | No se puede registrar una materia si el `id_profesor` asignado no existe previamente en la tabla `PROFESOR`. |
| **IR-02** | Para garantizar la relación uno a uno, el campo `matricula` en la tabla `CREDENCIAL` tiene una restricción `UNIQUE`. No puede haber dos credenciales asociadas a la misma matrícula. |
| **IR-03** | Al dar de baja a un alumno (`ALUMNO`), sus registros telefónicos en `ALUMNO_TEL` y su `CREDENCIAL` deben eliminarse en cascada (`ON DELETE CASCADE`) para evitar registros huérfanos. |
...
| Clave | Regla |
| :--- | :--- |
| **RN-01** | El campo `total_materias` visualizado en el DER no se implementa como columna física en la tabla `MATERIA` para cumplir con las reglas de normalización. Se calculará dinámicamente mediante consultas de agregación (`COUNT`). |
| **RN-02** | Los valores insertados en el campo `calif_final` dentro de la tabla `CURSA` deben oscilar de forma obligatoria en el rango permitido por el SGBD institucional (ej. 0.00 a 10.00). |
| **RN-03** | Un mismo alumno puede registrar múltiples números, pero no puede repetir el mismo identificador interno `id_telefono` en sus registros personales. |
