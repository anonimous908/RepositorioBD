...
| `jefe` | INT | - | FK, *Null* | `num_employee` del supervisor directo (Reflexiva). |
...
| `num_employee` | INT | - | FK, NN | Código del empleado proveedor del seguro/beneficio. |
...
| EMPLOYEE | `jefe` | EMPLOYEE(`num_employee`) |
...
| DEPENDENT | `num_employee` | EMPLOYEE(`num_employee`) |
...
| Clave | Regla |
| :--- | :--- |
| **RN-01** | La asignación jerárquica exige que el ID del `jefe` sea estrictamente diferente al `num_employee` del propio registro para evitar ciclos infinitos de autosupervisión. |
